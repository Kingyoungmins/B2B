...
namespace B2BSingleExe
{
    ...
    {
        private const string Version = "0.4.7";
        private const string ResourceName = "payload.zip";
        private const string MainExeName = "B2B_ver0.4.7.exe";
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                string extractDir = Path.Combine(
                    Path.GetTempPath(),
                    "B2B_ver0.4.7_single_" + Process.GetCurrentProcess().Id + "_" + DateTime.UtcNow.Ticks
                );
        ...
        private static void CleanupOldExtracts()
        {
            try
            {
                ...
                foreach (string dir in Directory.GetDirectories(temp, "B2B_ver0.4.7_single_*"))
                {
                    try
                    {
