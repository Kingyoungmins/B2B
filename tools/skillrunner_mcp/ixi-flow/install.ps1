# SkillRunner 설치 훅 — ixi-flow 설치기가 압축 해제된 번들 루트를 인자로 호출한다.
...
function Fail([string]$msg) { Write-Error "[skillrunner-install] $msg"; exit 1 }
...
$engine = Join-Path $BundleRoot "skillrunner\engine\serve_b2b.py"
...
$env:SKILLRUNNER_ENGINE_DIR = Join-Path $BundleRoot "skillrunner\engine"
$env:PYTHONUTF8 = "1"
$check = & $py -c "import win32com.client, skillrunner.runner_core as c; c._engine(); print('OK')" 2>&1
...
try {
    ...
    Write-Host "[skillrunner-install] Excel 감지: $($excel.'(default)')"
} catch {
    Write-Warning "[skillrunner-install] Excel 이 감지되지 않습니다. 스킬 실행(run_start)에는 Excel 설치가 필요합니다."
}

Write-Host "[skillrunner-install] OK"
exit 0
