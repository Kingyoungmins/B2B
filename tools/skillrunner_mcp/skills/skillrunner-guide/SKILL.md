---
name: skillrunner-guide
description: b2b 스킬(zip)을 SkillRunner MCP 도구로 실행하는 절차 — 입력 검사 → 실행 → 결과 압축. 사용자가 "스킬 실행", "스킬 돌려", "청구 스킬 처리" 등을 요청하면 이 절차를 따른다.
---

# b2b 스킬 실행 절차 (SkillRunner)

b2b 프로그램을 켜지 않고, 저장된 스킬(zip)을 `skillrunner__*` 도구로 직접 실행한다.
실행 PC 에 Excel 만 설치돼 있으면 된다.

## 절차 (반드시 이 순서)

### 1. 입력 검사 — `skillrunner__check_inputs`
...
- 종료 대기는 `bg_wait` 를 쓰면 된다(ixi-flow 가 `run_report` 를 자동 폴링).
  수동 확인은 `skillrunner__run_report` `{"run_id": "..."}` — `status` 가
...
- 별도 압축이 필요하면 `skillrunner__package_outputs`:
...
- 실행이 너무 오래 걸리거나 중단해야 하면 `skillrunner__run_stop` `{"run_id": "..."}`.
