# SkillRunner ixi-flow 배포 번들 빌드 (Windows 전용)
#
# 사용:
...
#
# 산출물: dist\skillrunner-deploy-<ver>-win64.tar.gz
#   → 배포 PC 에서: ixi-flow integrations install skillrunner --bundle <tar.gz>
#
...
$repo = (Resolve-Path (Join-Path $here "..\..")).Path   # tools\skillrunner_mcp → repo root

$pyExe = Join-Path $PythonDist "python.exe"
if (-not (Test-Path -LiteralPath $pyExe)) { throw "python.exe 가 없습니다: $pyExe" }

$staging = Join-Path $here "dist\_staging\skillrunner-deploy-$Version-win64"
...
# skillrunner 패키지 (+ 엔진 serve_b2b.py 를 패키지 안 engine/ 으로)
Copy-Item -Recurse (Join-Path $here "skillrunner") (Join-Path $staging "skillrunner")
New-Item -ItemType Directory -Force -Path (Join-Path $staging "skillrunner\engine") | Out-Null
Copy-Item (Join-Path $repo "serve_b2b.py") (Join-Path $staging "skillrunner\engine\serve_b2b.py")
...
Get-ChildItem -Recurse (Join-Path $staging "skillrunner") -Filter "__pycache__" -Directory |
...
$env:SKILLRUNNER_ENGINE_DIR = Join-Path $staging "skillrunner\engine"
$env:PYTHONUTF8 = "1"
$check = & $stagedPy -c "import win32com.client, skillrunner.runner_core as c; c._engine(); print('OK')" 2>&1
if ($LASTEXITCODE -ne 0 -or ($check -notmatch "OK")) { throw "self-check 실패: $check" }

Write-Host "[5/5] tar.gz 생성..."
$distDir = Join-Path $here "dist"
$tarPath = Join-Path $distDir "skillrunner-deploy-$Version-win64.tar.gz"
...
Write-Host "설치: ixi-flow integrations install skillrunner --bundle `"$tarPath`""
