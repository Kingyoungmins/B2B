# -*- coding: utf-8 -*-
"""SkillRunner MCP 서버 — ixi-flow 빌트인 MCP 계약(stdio JSON-RPC) 구현.
...
SERVER_INFO = {"name": "skillrunner", "version": "0.1.0"}
...
def _log(*a):
    print("[skillrunner-mcp]", *a, file=sys.stderr, flush=True)
...
def tool_run_start(args):
    ...
    with RUNS_LOCK:
        RUNS[run_id] = run
    t = threading.Thread(target=_run_worker, args=(run,), name="skillrunner-run", daemon=True)
    run["thread"] = t
    t.start()
...
def main():
    ...
    try:
        ...
        pass
    _log("skillrunner MCP server start (pid=%d)" % os.getpid())
