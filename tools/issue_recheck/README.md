...
## 자동화 계층 (2026-07-20: 완료 169건 전수 등록)

registry.json 에 지라 완료 이슈 169건 전부가 등록돼 있고, 체크는 세 계층이다:

1. **행위 테스트(node/python/com)** — 실제 로직/실제 Excel 로 증상 재현·검증. 가장 강함.
2. **마커 스모크(marker)** — `파일::문자열` 형식으로 그 이슈의 수정/기능 코드가 소스에
   존재하는지 검사. 기능요청·프롬프트 규칙·환경성 이슈처럼 행위 재현이 어려운 항목의 최소
   자동화로, 수정이 실수로 되돌려지면(revert) 즉시 잡는다. 등록 시점에 문자열 실존을 검증했다.
3. **repro 필드** — 재현 자산(고객 zip 등)의 로컬 경로 주석. 마커 항목을 행위 테스트로
   승격할 때의 재료다(승격 우선순위: repro 가 있는 marker 항목).

같은 실행 안에서 동일 체크는 1회만 돌고 결과가 재사용된다(여러 이슈가 한 테스트를 공유).

## 지라 완료 이슈 전수 관리

수동 점검 체크리스트(대시보드)는 자동화와 별개로 사람 확인 기록을 남긴다:

1. 지라 목록에서 CSV(현재 필드) 내보내기 → 대시보드 "목록 가져오기"에 붙여넣기 → 전수 목록 적재
   (재가져오기해도 기존 점검 상태는 키 기준으로 보존·병합된다)
2. 각 이슈에 [정상]/[재발]/[보류] 판정 + 메모(재발/보류 시 입력창) — 점검일 자동 기록
3. 상단 진행률 바 = (자동화 커버 + 수동 '정상') / 완료 전체. 필터: 미점검/자동화 없음/재발 등
4. 저장소: `jira_issues.json` (git 추적 — 점검 기록이 커밋으로 남는다)

레지스트리 항목의 id 가 SBAGENT-키와 같으면 자동화 커버로 집계된다. 슬러그 id 항목을 실제
지라 이슈에 연결하려면 registry.json 항목에 `"jira": "SBAGENT-000"` 필드를 추가하면 된다.
