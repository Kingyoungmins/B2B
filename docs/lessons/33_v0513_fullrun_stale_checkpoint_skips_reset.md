...
> ⚠️ **정정(2026-06-25, 저사양 트레이스):** 아래 "근본 원인"은 **반증됨**. SBAGENT-138 실제 원인은
> 체크포인트-skip 이 아니라 **리셋이 메모리에서만 복원·디스크 미저장 → 저사양이 디스크에서 재오픈해 stale(06_DAS)을 봄**.
> 정확한 분석·수정(`wb.Save()`)은 **lesson 35** 참조. Fix A/B 는 무해하나 이 버그를 못 막았다.
