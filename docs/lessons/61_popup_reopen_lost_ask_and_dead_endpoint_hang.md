...
## 원인 ③ — 도구 없이 답을 지어냄 (같은 날, 주소를 고친 뒤)

주소를 .108 로 고친 세션(15:02 재시작)에서 검산과 "마진율 최저 3곳" 질문에 모델이 **도구를 하나도 안 부르고**
`1,204,000` / `(주)삼영물산` 처럼 데이터에 없는 값을 답했다(실제 합계 3,949,012,800). 트레이스에 `assist.tool` 이
0건이라 바로 드러났다. 같은 코드·같은 모델로 내가 돌린 재현에선 data.query 를 4번 불렀다 — Qwen 의 비결정성.

**수정**: 루프 final 경로에 근거 가드 — 이번 턴 도구 호출이 0인데 답에 구체 수치(3자리 이상·소수·원/%)가 있고
질문/답이 데이터 얘기면 한 번 재촉한다("도구로 확인하거나, 못 하면 '확인하지 못했다'고 답하라. 지어내지 마라").
시스템 프롬프트의 날조 금지에도 '도구로 읽지 않은 수치는 말하지 마라'를 명시했고, 최종 답마다
`assist.final{tools:N}` 을 남겨 다음엔 "도구 몇 번 썼나"를 트레이스에서 바로 본다.

> 모델이 "확인했습니다"라고 말한다고 확인한 게 아니다. **근거는 루프가 세어야 한다** — 도구 호출 수가
> 0이면 그 답의 숫자는 전부 의심 대상이다.
