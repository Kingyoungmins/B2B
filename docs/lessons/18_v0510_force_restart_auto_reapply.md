...
```text
COM 작업이 N초 안에 끝나지 않았습니다.
```

(N은 서버가 채우는 가변 값이며, 클라이언트는 `/COM 작업이 .*초 안에 끝나지 않았습니다/` 패턴으로만 판정한다.)
...
3. **타임아웃 게이트는 정상(버그 아님)**: `noteExcelComTimeout`은 `COM 작업이 N초 안에 끝나지 않았습니다` 패턴일 때만, 그리고 `applying`/`preopening` 중이 아닐 때만 카운트한다. 최근 90초(`now - t < 90000`) 내 2회 이상(`recent.length >= 2`)이고 쿨다운(`forceRestartCooldownUntil`, 180000ms)이 지났을 때만 재시작한다. 멈춘 Excel을 살리는 동작 자체는 옳다 → **문제는 "살린 뒤 적용분 복원이 없다"는 것**.
...
- **재적용 조건은 둘 다 만족**: `wasApplied`가 true이고, 코드가 있고 활성(enabled)인 스텝이 하나라도 있어야(`hasSteps`) 한다. 둘 중 하나라도 아니면 즉시 return.
...
- `clearExcelMirrorClientState`는 미러 세션 매핑/타이머/플래그(`sessionsByFileId`, `activeExcelId`, `applying` 등)를 비우고 `invalidateLivePipelineApplied()`로 라이브 적용 시그니처도 무효화하지만, `state.inputs`/`state.pipeline`은 건드리지 않는다 → 재적용에 필요한 입력/스텝이 보존됨을 확인한다.
- 재오픈은 `hasFiles`(state.inputs/outputTemplates/output 중 존재)일 때만 수행됨을 확인한다.

자동 재적용 게이팅/루프가드/실패 처리는 Node 스모크 테스트로 검증한다(소스에서 `maybeAutoReapplyAfterRestart` 블록을 떼어내 목 전역으로 동작을 검증).

```powershell
node test_runs/_test_auto_reapply_after_restart.js
```
...
테스트가 검증하는 항목: wasApplied=false → 미실행, 활성 스텝 없음 → 미실행, 정상 경로 1회 실행+성공 토스트+applied 마감, 쿨다운 내 2회차 미실행+안내만, 1회 실패 후 재시도 없음+error 마감.
...
- `test_runs/_test_auto_reapply_after_restart.js` (자동 재적용 게이팅 검증)
