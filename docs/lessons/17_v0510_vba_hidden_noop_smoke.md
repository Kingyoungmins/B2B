...
> **갱신(2026-06-23): "변경 없음" 검증 자체를 전부 제거함.** 아래 화이트리스트(`_vba_allows_unchanged_fingerprint_success`)와
> 지문 비교(`_workbook_change_fingerprint`, 단일 적용 경로의 before/after 가드)는 삭제됐다. 서식만 바꾸는 작업(천단위 콤마/회계 서식 등)·
> 숨김 해제처럼 값/수식 지문에 안 잡히는 정상 작업까지 no-op 으로 오판해 "스킬을 적용하지 못했습니다"가 더 자주 떠서, 잡는 진짜 no-op 보다
> 막는 정상 작업이 많아 가드를 들어냈다. 이제 VBA 가 예외 없이 끝나면 성공으로 본다. **이 가드를 다시 추가하지 말 것**(필요하면 값 외에
> 서식/숨김/너비까지 보는 지문이어야 하는데 COM 비용이 커서 비권장). 아래 내용은 제거 전 동작의 역사 기록이다.
