...
def figure1():
    ...
    arrow(d, [(475, 415), (475, 530)], label="WebView2·좌표/상태")
    arrow(d, [(1925, 415), (1925, 530)], label="자식 프로세스")
    arrow(d, [(800, 720), (875, 720)], label="요구/매핑")
    arrow(d, [(1525, 720), (1600, 720)], label="HTTP")
    arrow(d, [(450, 950), (450, 1090)])
    arrow(d, [(500, 1090), (500, 950)])
    text_center(d, (475, 1020), "요청 / 코드", size=22, bold=True)
    arrow(d, [(1175, 950), (1175, 1090)])
    arrow(d, [(1225, 1090), (1225, 950)])
    text_center(d, (1200, 1020), "저장 / 호출", size=22, bold=True)
...
def figure2():
    ...
    box(d, (1300, 955, 700, 220), ["S205  실패 → 자동복구 (550)", "환경·캡처·기검증: 재생성 금지", "복구 가능: 체크포인트 복원 후 재생성·재적용"], fill=LIGHT2, size=26)
    arrow(d, [(1040, 1000), (1170, 1000), (1170, 837), (1300, 837)], label="성공")
    arrow(d, [(1040, 1015), (1300, 1015)], label="실패")
    arrow(d, [(1650, 915), (2070, 915), (2070, 1307), (1040, 1307)])
    arrow(d, [(1300, 1100), (1040, 1100)], dashed=True)
    text_center(d, (1170, 1132), "복원 후 재적용", size=20, bold=True)
...
def figure4():
    ...
    box(d, (260, 150, 1680, 125), ["S301  실행 집합 계산 및 1회 요청", "같은 파일의 연속 스텝 그룹화 · 실행 대상 ∪ 교차참조 파일"], fill=LIGHT, size=27)
    box(d, (260, 330, 1680, 165), ["S302  숨김 격리 Excel 인스턴스 1개 기동", "리셋 대상: 업로드 원본의 임시 사본(pristine)", "교차참조: 라이브 현재 상태의 SaveCopyAs 동반본"], fill=LIGHT2, size=26)
    box(d, (260, 550, 1680, 155), ["S303  스텝 루프", "[직전 SaveCopyAs 스냅샷] → [Python COM 또는 VBA 실행]", "진행률은 잠금 밖 공유 상태에 기록"], fill=LIGHT, size=26)
    for y1, y2 in [(275, 330), (495, 550)]:
        arrow(d, [(1100, y1), (1100, y2)])

    box(d, (150, 810, 850, 170), ["S304  성공", "변경 파일만 결과 반영", "라이브 또는 출력 파일로 전달"], fill=LIGHT2, size=27)
    box(d, (1200, 785, 850, 220), ["S305~S306  실패 및 최소 복구", "라이브 반영 전 예외 + 스냅샷 목록 동봉", "최초 스냅샷 복원 → 실패 스텝 재생성", "→ 실패 지점부터 이어 실행"], fill=LIGHT2, size=25)
    arrow(d, [(850, 705), (850, 755), (575, 755), (575, 810)], label="성공")
    arrow(d, [(1350, 705), (1350, 750), (1625, 750), (1625, 785)], label="실패")
    arrow(d, [(1625, 1005), (1625, 1080), (1100, 1080), (1100, 705)], label="복원 후 이어 실행", dashed=True)

    box(d, (150, 1100, 850, 185), ["S307  상태 표시 정합", "복원 세션 집합이 선행 스텝의 변형 문서 집합을", "모두 포함할 때만 선행 스텝을 ‘적용됨’으로 표시"], fill=LIGHT, size=25)
    arrow(d, [(575, 980), (575, 1100)], label="복구 후 커버리지 검증", dashed=True)
    text_center(d, (1100, 1370), "복구 상한: 전체 3회 · 스텝당 2회 / 스냅샷이 없으면 pristine 전체 재실행", size=23, fill=MID)
    text_center(d, (1100, 1430), "성공 전까지 라이브 세션은 변경하지 않으므로 원본과 라이브 상태를 무손실로 보존", size=24, bold=True)
...
def figure5():
    stages = [
        ...
        ("S503~S504  검증 및 결정적 스텝 생성", ["다중영역·크기·교차파일 검증", "paste_copied(src/dst book·sheet·range, values_only)", "역별칭으로 캡처 시점의 사용자 원본 문서명을 기록"], 205, LIGHT2),
        ("S505  결정적 재생", ["정방향 별칭으로 당시의 실제 문서를 해석·자동 오픈", "Range.Copy 또는 값만 붙여넣기", "값·수식·서식·병합 보존 / 롤백 범위 비용 한정"], 205, LIGHT),
    ]
