---
type: endpoint
title: _replaceStaleBookNamesInText
module: save-load.js
lang: js
extraction: regex   # 정규식 근사
signature: "(text, resolveName)"
role: "[프롬프트/설명 속 옛 달 파일명 교정] 파일확인 요구 추출은 꼬리표만이 아니라 스텝 prompt 의"
role_source: banner
version: "0.7.4"
loc: "save-load.js:788-788"

# ── 입출력 ──
inputs:
  - "text"
  - "resolveName"
returns: "(추정)"

# ── 사이드이펙트 (정적 추정) ──
side_effects:
  - "없음(정적 분석 기준)"
raises: []

# ── 유기적 관계 ──
calls:
  - "pipelineCollectWorkbookNames"
  - "resolveName"
calls_external:
  - "String"
  - "join"
  - "split"
  - "test"
  - "xls"
called_by:
  - "normalizeStaleBooksInSavedText"
  - "repairStalePromptBookNames"
reads: []
writes: []
affects: []                # (수동 보완) 정적 추출 불가 — 이게 틀어지면 깨지는 상위 기능
timestamp: "0.7.4-gen"
---

## 역할
[프롬프트/설명 속 옛 달 파일명 교정] 파일확인 요구 추출은 꼬리표만이 아니라 스텝 prompt 의

## 사이드이펙트 & 주의
- 없음(정적 분석 기준)

## 관계
- 호출: `pipelineCollectWorkbookNames`, `resolveName`
- 피호출(영향 전파 경로): `normalizeStaleBooksInSavedText`, `repairStalePromptBookNames`

## 실패/예외
- `(명시적 raise 없음/미탐지)`
