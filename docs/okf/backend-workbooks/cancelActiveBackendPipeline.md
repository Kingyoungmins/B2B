---
type: endpoint
title: cancelActiveBackendPipeline
module: backend-workbooks.js
lang: js
extraction: regex   # 정규식 근사
signature: "()"
role: "[중단] 협조적 python 잡 취소 — 실행 중 스텝은 끝까지 돌고, 다음 스텝 경계에서 멈춘다."
role_source: banner
version: "0.7.4"
loc: "backend-workbooks.js:772-772"

# ── 입출력 ──
inputs: []
returns: "(추정)"

# ── 사이드이펙트 (정적 추정) ──
side_effects:
  - "DOM/브라우저 전역 조작"
  - "네트워크/서버 호출"
raises: []

# ── 유기적 관계 ──
calls: []
calls_external:
  - "fetch"
  - "stringify"
called_by:
  - "_ensureExcelCancelButton"
  - "applyLogic"
  - "beginExcelMirrorApplyLoading"
  - "insertLogic"
  - "replaceLogicAt"
reads: []
writes: []
affects: []                # (수동 보완) 정적 추출 불가 — 이게 틀어지면 깨지는 상위 기능
timestamp: "0.7.4-gen"
---

## 역할
[중단] 협조적 python 잡 취소 — 실행 중 스텝은 끝까지 돌고, 다음 스텝 경계에서 멈춘다.

## 사이드이펙트 & 주의
- DOM/브라우저 전역 조작
- 네트워크/서버 호출

## 관계
- 호출: 없음
- 피호출(영향 전파 경로): `_ensureExcelCancelButton`, `applyLogic`, `beginExcelMirrorApplyLoading`, `insertLogic`, `replaceLogicAt`

## 실패/예외
- `(명시적 raise 없음/미탐지)`
