...
- 채팅 응답 스트리밍 중 코드블록은 처음부터 코드블록 영역에 표시하고, 청크 사이 멈칫거림이 덜 보이도록 렌더링을 완만하게 조정했습니다.
- 채팅 전송 후 대화창을 항상 아래로 정렬하고, 시뮬레이터의 드래그 선택 상태를 해제합니다.
- 원본 Excel 날짜 표시 문자열을 보존해 시뮬레이터에서 날짜 형식 셀이 더 자연스럽게 보이도록 보강했습니다.
- `test_data/README.md`에 실제 output 파일을 만들어가는 대화형 시연 시나리오를 추가했습니다.
