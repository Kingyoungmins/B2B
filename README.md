...
- **순수 Python(openpyxl) 스킬 실행 엔진 추가**: 스킬을 실제 Excel(COM) 대신 **openpyxl로 인프로세스 실행**하는 모드를 추가했습니다. COM 마샬링/Excel 오픈/단계별 디스크 저장 비용이 없어 크고 많은 파일에서 훨씬 빠릅니다.
  ...
  - **안전장치(자동 폴백)**: Python 엔진이 선택돼 있어도, 출력/입력 워크북에 **차트·이미지·피벗테이블·슬라이서·매크로(VBA)·수식**이나 **CSV**가 포함되면 그 실행만 자동으로 **Excel(COM) 엔진으로 전환**합니다(객체 유실·수식 미계산 방지). 전환되면 상단에 사유가 토스트로 표시됩니다. (xlsx zip 내부 경로/시트 XML을 가볍게 검사)
  - 셀 서식(글꼴·색·테두리·숫자서식·병합·열너비·조건부서식·데이터유효성)은 openpyxl 저장에도 그대로 유지됩니다. 유실되는 것은 위의 "객체"뿐입니다.
