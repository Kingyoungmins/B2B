...
### ver0.5.2.2 (Qwen 생성 품질 · 정적 게이트 오탐 · 스킬 토글/복구 무결성)

ver0.5.2 위에서, 실사용 중 보고된 "생성이 장황/멍청하다 · 정적 게이트가 정상 코드를 막는다 ·
스킬 ON/OFF 가 꼬인다 · 초기화/복구가 멈추거나 느리다" 계열 문제를 묶어 수정한 작업입니다.
구현 상세는 `docs/VER_0_5_2_2_REVISE.md` 참고.

- **Qwen3.6 생성 품질 개선** — 항상 1.5 로 걸던 `presence_penalty` 가 코드 토큰(`ctx.`, `Range`,
  `def` 등)의 정상 재사용까지 벌점을 줘 장황한 우회 표현을 유발하던 것을 기본 0.5 로 낮추고,
  줄 반복(degenerate)이 실제 감지된 재생성에서만 1.5 를 겁니다. think 모드 `max_tokens` 를
  8192 로 늘려(본문 잘림→no-think 폴백 방지) 주고, 대화 히스토리를 18/32k→12/20k 로 줄이며
  **오래된 assistant 턴의 코드 블록을 한 줄로 접어** 모델이 자기 과거의 장황한 출력을 모방하지
  않게 했습니다.
- **"설명만/주석만" 응답 자동 교정** — 함수를 만들지 않고 설명만 하거나 `#` 주석만 늘어놓는
  응답(코드 블록 없음 · 주석/뼈대뿐 · 진입점 없음)을 감지해 교정 지시와 함께 자동 재생성합니다
  (최대 2회). 정당한 되물음(어느 시트인가요? 등)은 재생성하지 않습니다.
- **Python COM 2회 실패 시 VBA 전환** — 정적 게이트는 2회(최초+재생성 1회) 통과 실패 시 바로
  VBA 로 전환 생성하고, 런타임 실행 실패도 같은 step 에서 2회째부터 VBA 매크로로 전환해 복구를
  시도합니다(전역 엔진 설정은 불변, 이 호출 1회만 VBA 프롬프트 사용).
- **정적 게이트 오탐 제거 (Python 스킬이 부당하게 막히던 핵심 원인)** —
  ① 루프 뒤에 오는 `ctx.write()`(권장 패턴)를 루프 안으로 오인하던 정규식을 들여쓰기 인식으로
  교정, ② 루프 안의 일반 리스트 `.copy()/.sort()/.clear()` 를 `ctx` 쓰기로 오인하던 것을
  `ctx`/`ctx.book(...)` 수신자에만 적용하도록 수정(서버 AST·클라 정규식 양쪽), ③ `re.compile()`
  같은 제공 모듈 메서드를 빌트인 `compile()` 로 오인하던 것 수정, ④ 게이트 검사 전 **주석을 제거**해
  금지 규칙을 메아리친 주석(`# openpyxl 금지` 등)이 오탐되지 않게, ⑤ `while 1` 도 차단.
  또한 샌드박스에 `chr/ord/divmod/map/filter` 를 추가해 열 문자 계산 코드가 런타임에 죽지 않게
  하고, 프롬프트의 `ctx.sort` key_col 의미와 27열(AA) 이상 안전한 열 문자 헬퍼를 보강했습니다.
- **스킬 ON/OFF · 삭제 무결성** —
  ① 재동기화가 관련된 **모든 워크북**(꺼진 스텝의 대상 + 코드가 파일명으로 참조하는 출력 파일까지)을
  각자 원본으로 리셋하고, 각 스텝을 **자기 대상 파일 세션**에서 실행하도록 고쳐 입력↔출력이 섞인
  파이프라인의 토글 꼬임을 해소. ② 대상 파일의 Excel 세션을 못 구하면 **현재 탭 워크북으로 조용히
  폴백하지 않고 중단** — "적용됐다는데 출력칸은 비고 엉뚱한 파일이 오염"되던 사고(탭 이동 후 ON
  미반영)의 원인 제거. ③ 적용 상태가 변하지 않는 편집(이미 OFF 인 스킬 삭제 등)은 느린 전체
  리셋+재적용을 **건너뜀**(no-op 생략). ④ 라이브 반영 실패 시 토글/삭제 UI 를 원복해 표시-실제
  불일치를 방지.
- **복구 장기화·서버 다운 회귀 수정** — 다중 파일 재동기화가 호출마다 동반 워크북 전체를
  재스냅샷해 복구가 수 분씩 걸리고 서버가 뻗던 문제를, 단일 파일 파이프라인은 기존 1회 호출
  빠른 경로로 처리하고 다중 파일은 같은 세션 스텝을 배칭하며, 서버는 **스텝 없는 순수 리셋 호출에서
  동반 스냅샷을 생략**하도록 고쳐 해소했습니다.
- **업로드 중 UI 차단** — 입력/출력 업로드의 파싱~미러 오픈 전 구간을 전역 busy 게이트로 잠가
  탭 전환·엑셀 뷰 클릭이 끼어들어 상태가 어긋나는 것을 막고, 중단은 오버레이의 "업로드 중단"
  버튼으로 받습니다.
- **초기화 멈춤 수정** — `confirm()` 블로킹 다이얼로그가 항상-위 Excel 미러 창 뒤에 가려져 화면이
  굳던 것을 미러를 먼저 숨기는 **비차단 DOM 모달**로 교체하고, 초기화(전부 폐기) 시 진행 중 작업이
  쥔 전역 busy 잠금을 즉시 강제 해제(`forceReleaseUiBusy`)해 failsafe(90초)까지 잠기던 문제를
  없앴습니다.
