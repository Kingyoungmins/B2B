...
- **수식 보존 표시 보강**: Excel(COM) 적용 결과를 프런트 미리보기로 반영할 때 수식 주소/수식 텍스트를 함께 갱신해, 복사·삽입·붙여넣기 후 수식이 값처럼 풀려 보이는 문제를 방지했습니다.
- **실패 Step 교체 흐름 보정**: 이전 실패 Step이 남아 새 요청 적용을 계속 막지 않도록, 다음 채팅 응답은 현재 활성 파일/시트/선택 범위를 기준으로 생성하고 적용 시 실패 Step을 교체하도록 처리합니다.
- **복사/붙여넣기 수식 보존 강화**: 복붙 요청에서 값만 복사하는 코드(`dest.Value = src.Value`, `PasteSpecial(xlPasteValues)`)나 단일 셀 COM 오류 패턴을 적용 전에 차단하고, 원본/붙여넣은 표의 수식과 서식을 유지하도록 프롬프트를 강화했습니다.
