...
namespace B2BNativeHost
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (System.Threading.Mutex instanceMutex = new System.Threading.Mutex(true, "B2B_NativeHost_SingleInstance_v1", out createdNew))
            {
                if (!createdNew)
                {
                    try { MessageBox.Show("B2B 스마트 빌링 에이전트가 이미 실행 중입니다.\n기존 창을 사용해 주세요.", "B2B 스마트 빌링 에이전트", MessageBoxButtons.OK, MessageBoxIcon.Information); }
                    catch { }
                    return;
                }
    ...
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            startupSplash.Text = "B2B 스마트 빌링 에이전트 준비 중입니다...\n\n처음 실행은 컴퓨터 성능에 따라 10~20초 정도 걸릴 수 있습니다.";
        ...
        private void UpdateNativeFileTabs(string message)
        {
            ...
            try
            {
                ...
                hint.Text = "📑 더블클릭으로 전환";
                hint.AutoSize = true;
                hint.ForeColor = Color.FromArgb(26, 26, 26);
                hint.BackColor = Color.FromArgb(242, 242, 242);
