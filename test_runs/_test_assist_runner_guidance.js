...
// [오진 정정 2026-08-26] 실행기 전체실행에는 이어실행이 없다(항상 1단계부터) —
// "앞 단계는 건너뜁니다"는 거짓 안내였다. 사용자에게 다시 나가지 않게 잠근다.
check("거짓 안내(앞 단계 건너뜀)를 하지 않는다",
  !/앞 단계는 건너뜁니다/.test(ui) && !/앞 단계는 건너뜁니다/.test(popup) && !/재사용해 건너뛰므로/.test(core));
check("지시문이 '항상 1단계부터'를 명시", /실행기 전체실행은 \*\*항상 1단계부터\*\*/.test(core));
