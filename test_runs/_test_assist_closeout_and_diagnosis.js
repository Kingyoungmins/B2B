...
check("한도인데 또 도구를 부른 경로", /if \(visible\) \{ assistPushAssistant\(visible, ui\); return; \}\s*\n\s*if \(await assistCloseOut\(\)\) return;/.test(ac));
check("예산 초과 break 는 루프 뒤 마무리로 떨어진다(별도 종료 아님)",
  /say\("시간이 오래 걸려 여기서 정리합니다\."\);\s*\n\s*break;/.test(ac));

console.log("[A-3] 마무리가 낸 결과를 실제로 쓴다");
check("본문이 있으면 말풍선으로 남긴다", /if \(body\) \{ assistPushAssistant\(body, ui\); return true; \}/.test(ac));
check("설계 채팅 넘김(handoff)도 살린다", /if \(cp\.action === "handoff"\)[\s\S]{0,320}ui\.onHandoff && ui\.onHandoff\(\{ request: req, reason: rsn \}\)/.test(ac));
check("마무리가 또 도구를 부르면 답으로 안 친다(무한 반복 방지)",
  !/cp\.action === "tool"[\s\S]{0,80}return true/.test(ac));
...
check("④ 읽기 없는 부분 갱신이면 원인 확정", /부분 갱신인데 읽기가 없으면 보존이\s*\n\s*불가능하므로 그것만으로 원인이 확정된다/.test(ac));
