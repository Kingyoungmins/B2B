...
console.log("[1c] 예고 뒤에 짧은 부연 한 문장  ← 2026-09-09 실측(검산 요청 후 멈춤, '?' 로 재촉)");
check("'…읽어서 맞춰 볼게요. 원본은 금액 열이에요.' → 예고",
  isDangling("매출 원본의 총합을 읽어서 맞춰 볼게요. 원본은 금액 열이에요.") === true);
check("'…확인해 보겠습니다. 잠시만요.' → 예고",
  isDangling("먼저 회사별요약의 합계 셀을 확인해 보겠습니다. 잠시만요.") === true);
check("예고 뒤에 '결과'가 실제로 붙으면 완결 — 통과",
  isDangling("총합을 계산해 볼게요. 결과: 매출 원본 총합은 169,403,600원이고 요약 합계와 일치합니다.") === false);
check("예고 뒤에 숫자 있는 문장이면 완결 — 통과",
  isDangling("합계를 맞춰 볼게요. 두 값 모두 1,234원입니다.") === false);

console.log("[1d] 동사 허용목록 밖의 예고  ← 2026-09-09 12:3x 실측('먼저 1단계 코드를 볼게요.' 에서 멈춤)");
check("'먼저 1단계 코드를 볼게요.' → 예고", isDangling("2단계가 조금 이상해요. 이 두 단계의 실제 코드를 봐야 어디서 2배가 됐는지 알 수 있어요.\n\n먼저 1단계 코드를 볼게요.") === true);
check("'그 파일을 열어 볼게요.' → 예고", isDangling("그 파일을 열어 볼게요.") === true);
check("'감사합니다, 좋은 하루 보내시길 바랄게요.' 인사 → 통과", isDangling("감사합니다, 좋은 하루 보내시길 바랄게요.") === false);
