...
#  - 단독 백엔드: BACKEND_DIR(b2b_backend_v044) 전체 무조건 삭제(나이/ auto_backups 예외 없음).
#  - Temp 루트(isopipe_/single_/_MEI_/Excel진단): stale 삭제 / recent 보존(동시실행 보호) — 종전과 동일.
#  - 동시 백엔드(케이스2): BACKEND_DIR 는 전이성 stale 만 정리, uuid_ 원본사본은 보존.
...
mk_dir(backend / "live_RECENT", NEW)              # 단독 백엔드=전체 삭제 → 최근이어도 삭제
mk_dir(backend / "auto_backups", OLD)             # 단독 백엔드=전체 삭제(진짜 백업은 앱 폴더에 있음)
...
mk_dir(fake / "B2B_ver0.6.2_single_OLD", OLD)    # 삭제
mk_dir(fake / "B2B_ver0.6.2_single_NEW", NEW)    # 보존(최근)
...
mk_file(fake / "_MEIB2BOWN" / "serve_b2b.py", NEW)
mk_file(fake / "Diagnostics" / "EXCEL" / "log_OLD.txt", OLD)  # 삭제
mk_file(fake / "Diagnostics" / "EXCEL" / "log_NEW.txt", NEW)  # 보존
# WebView2 사용자데이터(B2B_WebView2/verNNN_<pid>): 죽은 pid 삭제 / 살아있는 pid 보존
mk_dir(fake / "B2B_WebView2" / "ver044_99999999", OLD)               # 죽은 pid → 삭제
mk_dir(fake / "B2B_WebView2" / "ver043_99999997", OLD)               # 구버전 마커 죽은 pid → 삭제(넓힌 글롭)
...
checks = [
    ...
    ("live_RECENT 삭제(전체비움)", gone(backend / "live_RECENT")),
    ("auto_backups 삭제(전체비움)", gone(backend / "auto_backups")),
    ...
    ("single_OLD 삭제",          gone(fake / "B2B_ver0.6.2_single_OLD")),
    ("single_NEW 보존(최근)",    kept(fake / "B2B_ver0.6.2_single_NEW")),
    ...
    ("webview2 구버전 ver043 죽은 pid 삭제", gone(fake / "B2B_WebView2" / "ver043_99999997")),
...
mk_dir(fake2 / "B2B_ver0.6.2_single_OLD2", OLD)
...
checks2 = [
    ...
    ("[가드] single 임시 보존", (fake2 / "B2B_ver0.6.2_single_OLD2").exists()),
