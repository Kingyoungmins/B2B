...
def make_handler(lifecycle: BrowserLifecycle):
    class LifecycleHandler(B2BHandler):
        def do_POST(self):
            if self.path == "/__b2b_ping":
                lifecycle.ping(read_session_id(self))
                self.send_response(204)
                self.end_headers()
                return
            if self.path == "/__b2b_close":
                lifecycle.close(read_session_id(self))
                if not lifecycle.has_active_sessions():
                    cleanup_excel_sessions()
                self.send_response(204)
                self.end_headers()
                return
...
def start_server(port: int, lifecycle: BrowserLifecycle) -> ReusableThreadingTCPServer:
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="b2b-http-server", daemon=True)
    thread.start()
    return httpd
...
def start_lifecycle_monitor(lifecycle: BrowserLifecycle, shutdown_event: threading.Event) -> None:
    ...
    thread = threading.Thread(target=monitor, name="b2b-browser-lifecycle", daemon=True)
    thread.start()
...
def show_control_window(url: str, on_close) -> None:
    ...
    def handle_close():
        if messagebox.askokcancel("B2B 업무망", "B2B 업무망 서버를 종료할까요?"):
            root.destroy()
...
def main() -> int:
    ...
    try:
        ...
        if os.environ.get("B2B_NO_BROWSER") != "1":
            webbrowser.open(url)
        print(f"B2B launcher started: {url}")
        if os.environ.get("B2B_SHOW_CONTROL") == "1":
            show_control_window(url, shutdown_event.set)
            return 0
        if os.environ.get("B2B_NO_WINDOW") == "1" or os.environ.get("B2B_SHOW_CONTROL") != "1":
            shutdown_event.wait()
    except KeyboardInterrupt:
        print("\nStopping...")
        return 0
