...
## 2026-09-09 (오후) — 활성시간이 체류를 넘던 버그, 전체실행 중복 집계

- **왜**: 대시보드에서 '활성(추정) > 체류' 세션이 5개 발견(전부 +0.8분). 원인은 앱의 `telemetry_preview.jsonl` 이
  세션마다 리셋되지 않는 **누적 파일**인데 `log_sync` 가 세션 시작마다 오프셋을 0으로 돌려 **매번 통째로** 올린 것.
  9/1 의 agent.run 두 건(49초 간격)이 모든 세션에 딸려 들어와 활성 +0.8분, 전체실행 횟수는 세션마다 과거분까지 중복.
  게다가 그 파일의 `timestamp` 는 UTC(Z)인데 로컬로 읽어 9시간이 어긋나 있었다.
- **수정**(`collector.py`, 캐시 v6 — 기존 데이터 소급 교정):
  - `_parse_ts_seconds`: `Z`/오프셋은 UTC 로 변환.
  - `_session_window_seconds`: session.json 의 시작(−5분)~끝(+5분) 창. **누적 파일(`telemetry_preview.jsonl`)에 한해**
    창 밖 타임스탬프 기록은 활성·전체실행·토큰 집계에서 제외(`skippedOutOfWindow`). vba/runtime 트레이스는 앱 시작 때
    리셋되므로 필터 대상이 아니다(시계가 어긋난 PC 의 정상 로그를 버리지 않게).
  - 활성 ≤ 체류 클램프 — **종료된 세션(endedAt)에만**. 열린 세션은 끝=마지막 수신이라 로그보다 뒤처져 0 으로 잘렸다(실측).
- **실측**: 오늘 세션 전수 재계산 → 활성>체류 0개, 세션당 전체실행 1건(예전 5~7건). `test_active_time.py` 에 4b/4c 추가.
