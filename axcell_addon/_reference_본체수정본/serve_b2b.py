...
import b2b_scheduler   # [스케줄러 모듈] 스케줄 등록/목록 화면의 서버측
import b2b_telemetry   # [관측 로그] 전사 Agent Observability 수집
import csv
import ctypes
import datetime
import gc
...
class B2BHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.split("?")[0] == "/api/backend/health":
            ...
            return
        # [스케줄러 모듈] 이 모듈이 처리하는 경로면 여기서 끝난다. 아니면 None.
    ...
    def handle_excel_run_full_pipeline(self):
        ...
        # [관측 로그] 실행 1건을 기록한다. 스킬명은 클라이언트가 실어 보낸다.
        _tel_t0 = time.time()
