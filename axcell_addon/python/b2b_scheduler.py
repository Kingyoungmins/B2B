...
def _read_schedule(folder):
    ...
    if manifest.is_file():
        try:
            item.update(json.loads(manifest.read_text(encoding="utf-8")))
            item["dir"] = str(folder)
    ...
    try:
        item["files"] = [{"name": p.name, "bytes": p.stat().st_size}
                         ...
                         if p.is_file() and p.name not in ("cron.txt", "config.txt", "schedule.json")]
    except OSError:
        pass
    return item
...
def _folder_files(root):
    ...
    try:
        for f in sorted(root.iterdir()):
            if f.is_file() and f.name not in _PROTECTED_FILES:
...
def update_files(payload):
    ...
    remove_list = [r for r in remove_list if r and r not in _PROTECTED_FILES]
