...
// [0.5.1 이식] 사용자의 최신 메시지가 직전 결과에 대한 '정정'인지 추정(정정일 때만 정정-우선 지시 강화).
...
async function callLLM(userMessage, options) {
  ...
  // forceEngine: Python 정적 게이트를 연속 통과 못해 VBA 로 폴백 생성할 때 등,
  // 이 호출 1회에 한해 시스템 프롬프트 엔진을 강제한다(전역 엔진 설정은 불변).
  ...
  if (engine === "python" && typeof PYTHON_COM_SYSTEM_PROMPT === "string") {
    // ver0.5.2 4단계: Python COM bulk 엔진(기본) — ctx API 전용 프롬프트.
  ...
  // [0.5.1 이식] 최신 메시지가 정정이면 이전 코드/해석을 고집하지 말고 정정을 최우선 반영하도록 강조.
...
async function callOpenAICompat(system, options) {
  ...
  for (let attempt = 1; attempt <= OPENAI_COMPAT_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      // [0.5.1 이식] Qwen3.6 think 반복(컨텐츠 없이 reasoning만 무한) 감지 시 no-think로 1회 자동 폴백.
...
async function callOpenAICompatOnce(system, options) {
  ...
  // Qwen3 계열은 공식 가이드가 greedy/저온 디코딩을 금지한다 — temperature 0.2 같은 준-greedy
  // 설정에서 같은 줄을 끝없이 반복하는 degenerate 출력이 발생한다(FP8 양자화는 더 심함).
  // 권장값: no-think temp 0.7/top_p 0.8, think temp 0.6/top_p 0.95, top_k 20,
  // 양자화 모델 반복 억제용 presence_penalty(0~2, 권장 1.5).
...
// [0.5.1 이식] 저장된 baseUrl 변형(빈값/옛 직접주소/로컬 변형)을 네트워크별 기본값으로 정규화.
...
async function readOpenAICompatStream(resp, options) {
    ...
    for (const line of lines) {
      ...
      if (data === "[DONE]") {
        if (!full.trim() && reasoningFull.trim()) {
          // [0.5.1 이식] reasoning만 흘리고 본문 없이 종료 → no-think 자동 재시도 신호.
      ...
      if (allowReasoning && reasoningDelta) {
        ...
        // [0.5.1 이식] 본문 없이 reasoning 만 길게 반복되면(루프) 끊고 no-think 재시도.
  ...
  if (!full.trim() && reasoningFull.trim()) {
    // [0.5.1 이식] 스트림이 [DONE] 없이 끝났는데 본문이 없으면 think 폴백 대상.
    const err = new Error("reasoning stream ended without content");
    err.retryWithoutThink = true;
    throw err;
  }
...
// [0.5.1 이식] reasoning 꼬리 구간의 반복 패턴(같은 문장 블록 3회 이상 등)을 휴리스틱으로 감지.
...
async function fetchOpenAICompat(path, preferredBase, options = {}) {
  // 로컬 ixi 프록시(/v1) 경유일 때만, 서버가 전달할 실제 Violet/vLLM 주소를 헤더로 알려준다.
  // 프록시(서버)만 이 헤더를 읽으며, 개발망 vLLM/직접 주소 연결 시에는 붙이지 않는다.
  const preferred = String(preferredBase || "").trim().replace(/\/$/, "");
  const usingLocalProxy = settings.provider === "openai-compat"
    && settings.network !== "dev-vllm"
    && typeof isLocalIxiProxyBaseUrl === "function"
    && isLocalIxiProxyBaseUrl(preferred);
  const upstream = usingLocalProxy ? (settings.proxyUpstream || "") : "";
  if (upstream) {
