...
// 자동 바닥 스크롤은 "사용자가 바닥 근처에 있을 때"만 동작한다.
// 스트리밍 중 위로 스크롤해 이전 내용(중단 버튼 포함)을 보려는데 매 토큰마다
// 바닥으로 끌려 내려가던 문제(구버전 신고: '처리중 스크롤 불가') 방지.
let chatAutoScrollPinned = true;

function installChatScrollTracking(container) {
  if (!container || container.dataset.autoScrollTracked) return;
  container.dataset.autoScrollTracked = "1";
  container.addEventListener("scroll", () => {
    // 바닥에서 48px 이내면 '바닥에 붙어 있음' → 자동 스크롤 유지.
    chatAutoScrollPinned = (container.scrollHeight - container.scrollTop - container.clientHeight) < 48;
  });
}
...
function scrollChatToBottom(force = false) {
  const container = $("chat-messages");
  if (!container) return;
  installChatScrollTracking(container);
  if (force) chatAutoScrollPinned = true; // 사용자 전송 등 명시 동작은 항상 바닥으로
  if (!chatAutoScrollPinned) return;
...
// Python COM 스킬이 같은 step 에서 런타임 오류로 반복 실패하면(정적 게이트와 별개),
// Python COM 기반 자체의 제약일 가능성이 높다고 보고 VBA 생성으로 전환한다.
...
async function requestErrorRecovery(stepIdx, errorInfo) {
  ...
  // Python COM 런타임 실패가 같은 step 에서 누적되면(기본 2회) Python COM 기반 자체의
  // 제약으로 판단하고 이번 복구부터 VBA 전환 생성을 시도한다(전역 엔진 설정은 불변).
  ...
  const failedCodeLang = vbaRuntimeSwitch
    ? "python"
  ...
  try {
    const requestOptions = {
      ...
      // VBA 전환 복구는 이 호출 1회만 VBA 시스템 프롬프트를 쓴다(전역 엔진 설정 불변).
