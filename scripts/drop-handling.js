...
function runnerExtractMappingRequirements() {
  const map = new Map();
  // [v4 요구 테이블 우선] 불러온 스킬이 저장 시점에 계산한 requiredFiles(pristine 업로드 ∩ 사용 파일)가
  // 있고, 그 표가 '지금 파이프라인'과 여전히 일치하면(서명 동일), 코드 추론 대신 선언표를 요구 목록으로
  // '직접' 쓴다. 생성 시트는 requiredSheets 에 없으므로 실행기 화면에서도 자연히 제외된다.
  // 생성기에서 편집돼 서명이 달라지면(또는 구버전 zip/신규 작성) 아래 기존 추론으로 폴백한다 — 편집·이력
  // 기능은 그대로 유지되고, 낡은 요구를 보여주지 않는다.
  const v4t = (typeof state !== "undefined" && state && state.loadedSkillRequirements) || null;
  const sigOk = !v4t || state.loadedSkillReqPipelineSig == null
    || (typeof loadedSkillReqSignature === "function"
        && loadedSkillReqSignature(state.pipeline) === state.loadedSkillReqPipelineSig);
  if (v4t && sigOk && Array.isArray(v4t.requiredFiles) && v4t.requiredFiles.length) {
    for (const rf of v4t.requiredFiles) {
      const book = rf && rf.name ? String(rf.name) : "";
      if (!book) continue;
      const sheets = Array.isArray(rf.requiredSheets) ? rf.requiredSheets.filter(Boolean) : [];
      if (sheets.length) {
        for (const sh of sheets) runnerAddRequirement(map, book, sh, "v4");
      } else {
  ...
  // [v4 요구 테이블] 표가 있어도(서명 불일치로 폴백해 여기까지 온 경우 등) 저장 시 계산된 unresolvedRefs
  // (pristine 업로드와 매칭 안 된 코드 내 이름 = 대부분 excel_open_* 복붙 캡처 임시명)를 요구 목록에서
  // 제거한다. 임시명 패턴에 정확히 맞는 것만 지워 실제 파일명이 잘못 걸러지지 않게 한다.
  const v4req = (typeof state !== "undefined" && state && state.loadedSkillRequirements) || null;
  if (v4req && Array.isArray(v4req.unresolvedRefs) && v4req.unresolvedRefs.length) {
    const isTempName = n => /^(?:excel_open_[0-9a-f]{8,}|live_reset_[0-9a-f]{8,}|[0-9a-f]{12,})/i.test(String(n || "").trim());
    const ghosts = new Set(v4req.unresolvedRefs.filter(isTempName).map(n => runnerMappingNorm(n)));
    if (ghosts.size) {
      for (const [key, req] of Array.from(map.entries())) {
        if (req.book && ghosts.has(runnerMappingNorm(req.book))) map.delete(key);
      }
    }
  }
  return Array.from(map.values()).slice(0, 40);
...
// [바인딩 변수] v4 스킬의 requiredFiles 에 배정된 파일 핸들(예: "@@FILE_1@@")을 origName 으로 찾는다.
// 핸들 스킬이면 코드엔 실제 파일명 대신 이 핸들이 박혀 있어, 실행 시 매핑된 실제 파일명으로 복원한다.
// (리터럴 스킬이면 handle 필드가 없어 "" 반환 → 아무 일도 안 함. 하위호환.)
function runnerHandleForName(name) {
  const v4 = (typeof state !== "undefined" && state && state.loadedSkillRequirements) || null;
  if (!v4 || !Array.isArray(v4.requiredFiles) || !name) return "";
  const want = runnerMappingNorm(name);
  for (const rf of v4.requiredFiles) {
    if (rf && rf.handle && runnerMappingNorm(rf.name) === want) return String(rf.handle);
  }
  return "";
}
...
window.buildRunnerMappedPipeline = function(steps) {
  ...
  return (steps || state.pipeline || []).map(step => {
    ...
    rows.forEach(row => {
      ...
      if (row.req.book && actualName) {
        code = runnerReplaceLiteral(code, row.req.book, actualName);              // (리터럴 스킬) origName → 실제
        const fileHandle = runnerHandleForName(row.req.book);                     // [바인딩 변수] (핸들 스킬) @@FILE_n@@ → 실제
        if (fileHandle) code = runnerReplaceLiteral(code, fileHandle, actualName);
      }
