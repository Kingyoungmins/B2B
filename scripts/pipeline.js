...
async function runIsolatedLivePipelineSteps(sourceSteps, initialExcelId, options = {}) {
  ...
  try {
    ...
    for (const group of groups) {
      ...
      if (!skipReset && !resetDone.has(group.fileId)) {
        const resetData = await postExcelMirror("/api/excel/run-vba-pipeline", {
          excelId,
          steps: [],
          reset: true,
      ...
      for (const stepPayload of group.steps) {
        ...
        if ((skipReset || resetDone.has(group.fileId)) && originalStep) {
          const snap = await captureStepPreApplySnapshot(originalStep, excelId);
          syncStepPreApplySnapshot(originalStep, snap, Number.isInteger(stepPayload && stepPayload.stepIdx) ? stepPayload.stepIdx : null);
          if (!snap || !snap.resultId) {
            throw createPipelineStepError(
              Number.isInteger(stepPayload && stepPayload.stepIdx) ? stepPayload.stepIdx : -1,
              originalStep,
  ...
  state.pipeline.forEach((step, idx) => {
    ...
    item.querySelector(".step-toggle").onclick = async (e) => {
      ...
      if (Number.isInteger(resumeBeforeToggle) && currentIdx >= resumeBeforeToggle) {
        ...
        if (typeof toast === "function") toast("보류 구간의 ON/OFF만 변경했습니다.", "success");
        return;
      }
      if (fastLast) {
        try {
      ...
      if (repairedStep && pipelineStepLiveLanguage(repairedStep)) {
        try {
          ...
          throw createPipelineStepError(
            stepIdx,
            repairedStep,
            "자동 복구는 완료했지만 실패 Step 직전 스냅샷이 없어 이어 실행을 중단했습니다. 전체실행 반복으로 인한 중복 적용을 막기 위한 중단입니다.",
            "checkpoint snapshot missing after auto repair"
          );
        } catch (resumeErr) {
