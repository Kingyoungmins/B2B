...
- \`ctx.native_pivot("시트", group_by=..., value=..., agg="sum", dest_name=..., column=None)\` → **엑셀 '진짜 피벗테이블(PivotTable 개체)'** 을 새 시트에 만듦(원본과 연결돼 **새로 고침**되고 필드 배치가 살아있는 피벗). 인자는 ctx.pivot 과 동일(그룹키/값 **다중 리스트 지원**, value=None 은 개수, column 은 열 필드). **기본은 값-표인 ctx.pivot 을 쓰고**, 사용자가 **"진짜 피벗/피벗테이블로 만들어줘/새로고침 되는/슬라이서"** 를 명시할 때만 native_pivot 을 쓰세요. 예: \`ctx.native_pivot("매출", group_by=["회사","지점"], value=["금액","건수"], agg=["sum","count"], dest_name="피벗")\`. (COM 특성상 값-표 피벗보다 드물게 실패할 수 있으니, 특별한 요청이 없으면 ctx.pivot.)
