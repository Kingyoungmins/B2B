...
- 사용자가 "대상 P열의 한 셀에 여러 값이 있고, 그 값들이 입력 BP열과 일치하면 입력 BQ열 값을 합계해 대상 H열에 작성"처럼 요청하면 Python COM으로 처리하세요. VBA 매크로/피벗이 필요 없습니다.
...
- \`ctx.write(시트, "B2", 이차원리스트, overwrite_formulas=False)\` → 시작 셀 기준 한 번에 기록
  - **수식 셀에 값 덮어쓰기**: 사용자가 "값/값만/덮어/입력/채워"처럼 값 채우기를 요청하면, 대상 셀에 수식이 있어도 \`overwrite_formulas=True\` 로 값을 그대로 덮어쓰세요(수식 셀을 건너뛰거나 제외하지 마세요). 수식을 보존해야 하는 경우는 사용자가 "수식 유지/수식 건드리지 마"라고 **명시**했을 때뿐입니다.
...
- 입력 파일에서는 BP 키 → BQ 숫자 합계 딕셔너리를 먼저 만드세요. BQ가 숫자 문자열이면 쉼표를 제거해 숫자로 더하고, 빈칸/문자는 0으로 처리하거나 건너뛰세요.
- 출력은 매칭된 행의 H 셀만 갱신하세요. 매칭이 없는 행, H열의 기존 텍스트("해지" 등), 합계/소계 수식 행은 그대로 둡니다. H열 전체를 1열 배열로 다시 쓰면 기존 금액/수식이 0 또는 값으로 오염됩니다.
- 수식 행 보호가 필요하면 \`ctx.formula_mask("SO사업자별요금", f"H{start}:H{last}")\` 로 수식 여부를 확인하고, True인 행은 쓰지 마세요. 매칭 행이 소수이면 \`ctx.write_cell(sheet, f"H{row}", total, overwrite_formulas=True)\` 로 해당 셀만 쓰는 것이 안전합니다.
- 사용자가 명시적으로 "없는 값은 0으로"라고 하지 않으면 매칭 없는 행을 0/빈칸으로 덮어쓰지 마세요.
...
- ctx.write 는 대상에 수식이 있으면 기본적으로 차단됩니다. 사용자가 "값/값만/값 복사/값으로 붙여넣기/덮어쓰기/입력/채워"처럼 값 쓰기를 **명시**하면 \`overwrite_formulas=True\` 로 해당 대상 범위에 한정해서 값으로 덮어쓰세요. 단순한 전체 UsedRange 재기록으로 무관한 수식을 없애면 안 됩니다.
