...
- **병합셀 위에서 구조 작업(열 이동/맞바꿈·Cut·ClearContents)을 그대로 하면 \`병합된 셀에서는 실행할 수 없습니다\`(런타임 1004)로 통째 실패해 한 셀도 안 바뀝니다.** 제목/헤더 병합(예: \`A1:E1\`, \`D4:D5\`)이 걸린 범위를 다룰 땐:
  - 셀 비우기는 MergeArea 단위로: \`For Each c In rng.Cells: If c.MergeCells Then c.MergeArea.ClearContents Else c.ClearContents: Next c\`.
  - 열 맞바꿈/이동은 **① 대상 영역 \`MergeArea.UnMerge\` → ② 값+서식 교환/이동 → ③ 필요하면 원래대로 \`Range(...).Merge\` 재병합** 순서로 하세요. 헤더 병합을 안 풀고 전체 열을 Cut/Insert 하면 1004 입니다(열 "삽입·삭제"만은 전체 열 단위라 병합 안전 — 위 참고).
  - **이 1004 를 \`On Error Resume Next\` / \`Err.Clear\` 로 삼키지 마세요.** 삼키면 변경 0건인데도 "적용됨"으로 보입니다. 1004 가 나면 위 패턴으로 실제 수행하거나, 정말 불가하면 \`Err.Raise\` 로 실패를 알리세요. (조건에 맞는 행이 0건이라 안 바뀌는 정상 케이스는 오류가 아니므로 제외.)
