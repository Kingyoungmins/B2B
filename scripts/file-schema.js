...
- \`ctx.pivot("시트", group_by="회사", value="금액", agg="sum", dest_name="회사별합계")\` → 그룹별 집계 피벗을 **새 시트(활성 파일)**에 만듦(원본 보존). agg 는 sum/count/avg/max/min. **기본은 엑셀 '진짜 피벗테이블(PivotTable 개체, 새로고침 가능)'로 만들고, 실패하면 자동으로 '값 표'로 폴백**합니다 — 호출부는 ctx.pivot 하나만 쓰면 됩니다.
...
- \`ctx.native_pivot("시트", group_by=..., value=..., agg="sum", dest_name=..., column=None)\` → **엑셀 '진짜 피벗테이블(PivotTable 개체)'** 을 새 시트에 만듦(원본과 연결돼 **새로 고침**되는 살아있는 피벗). 인자는 ctx.pivot 과 동일(다중 키/값 리스트, value=None=개수, column=열 필드). **보통은 ctx.pivot 만 쓰면 됩니다**(ctx.pivot 이 이미 기본으로 이 진짜 피벗을 만들고 실패 시 값-표로 폴백). 값-표 폴백을 원치 않고 **반드시 진짜 피벗만** 만들어야 할 때(폴백 없이)만 native_pivot 을 직접 부르세요.
