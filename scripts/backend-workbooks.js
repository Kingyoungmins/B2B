...
// [0.5.1 이식] 적용 직전의 현재 view(파일/시트/선택)를 캡처해, 적용 후 임의로
// output/activeSheet 로 탭이 튀지 않게 복원한다(merge plan §4 view restore).
...
function applyBackendPipelineResult(result) {
  ...
  // [0.5.1 이식] result.activeSheet 는 "어느 시트가 바뀌었는지"에 가까운 실행 메타다.
  // 여기서 현재 탭을 바꾸면 적용 후 사용자가 보던 입력/출력/시트가 임의로 이동한다.
  // 대신 적용 직전 캡처한 view(restoreView)를 기준으로 파일/시트/선택을 복원한다.
  ...
  if (result && result.pythonExcel) {
    // [0.5.1 이식] 단순화: restoreView 가 복원한 "지금 보고 있는 파일" 하나만 즉시 갱신/표시하고
    // (세션 유실 시에만 그 파일 하나를 재오픈 — 전체 close/reopen 금지),
    // 나머지 변경 파일은 stale 표시만 해 두고 그 파일로 전환할 때 갱신한다.
...
async function runPipelineOnBackend(options = {}) {
  ...
  // [0.5.1 이식] 적용 직전 view 캡처 — 결과 반영 후 이 view 기준으로 복원한다.
