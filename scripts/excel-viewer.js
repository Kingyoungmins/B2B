...
function _renderViewerInitial(viewer, file) {
  ...
  const visibleCols = Math.min(sourceCols, VIEWER_PREVIEW_COLS);
  const totalRows = Math.min(renderableRows, VIEWER_PREVIEW_ROWS);
  const initialRows = Math.min(totalRows, VIEWER_PREVIEW_ROWS);
...
function updateChatRangeReference(range, options = {}) {
  ...
  if (options.append) {
    ...
    if (!options.preserveFocus) ta.setSelectionRange(ta.value.length, ta.value.length);
    return;
  }
  let last = null;
  let match;
...
function commitCellEdit(fileId, sheet, r, c, value) {
  ...
  try {
    ...
    renderPipeline();
    refreshRunButton();
    if (typeof scheduleLogicAutoBackup === "function") scheduleLogicAutoBackup("manual-edit");
